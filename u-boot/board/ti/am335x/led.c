...
#define BEAGLE_LED_USR0	GPIO_TO_PIN(1, 21)
#define BEAGLE_LED_USR1	GPIO_TO_PIN(1, 22)
#define BEAGLE_LED_USR2	GPIO_TO_PIN(1, 23)
#define BEAGLE_LED_USR3	GPIO_TO_PIN(1, 24)
...
void __led_set (led_id_t mask, int state)
{
	...
	if (STATUS_LED_BIT2 & mask) {
		if (!gpio_request(BEAGLE_LED_USR2, "")) {
			gpio_direction_output(BEAGLE_LED_USR2, 0);
			gpio_set_value(BEAGLE_LED_USR2, state);
		}
	}
#endif

#ifdef STATUS_LED_BIT3
	if (STATUS_LED_BIT3 & mask) {
		if (!gpio_request(BEAGLE_LED_USR3, "")) {
			gpio_direction_output(BEAGLE_LED_USR3, 0);
			gpio_set_value(BEAGLE_LED_USR3, state);
		}
	}
#endif
