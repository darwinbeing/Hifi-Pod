...
{
public:
	Data( const BYTE *data, const DWORD len )
		: data( data ), dataStart( data ), dataEnd( data+len )
	{}
	...
	DWORD getDWORD()
	{
		...
		DWORD ret = data[ 0 ] | data[ 1 ] << 8 | data[ 2 ] << 16 | data[ 3 ] << 24;
		data += sizeof ret;
		return ret;
	}

	Data & operator = ( const DWORD pos )
	{
	...
	Data operator + ( const DWORD bytes ) const
	{
	...
	BYTE operator [] ( const DWORD idx ) const
	{
		if ( atEnd() )
			return 0;
		return data[ idx ];
...
private:
	const BYTE *data, * const dataStart, *const dataEnd;
};
...
static inline DWORD FourCC( const char str[ 5 ] )
{
...
public:
	...
	{
		Data data( lpStream, dwMemLength );
		while ( !data.atEnd() )
		{
...
BOOL CSoundFile::ReadJ2B( const BYTE *lpStream, DWORD dwMemLength )
{
	...
	for ( DWORD i = 0 ; i < riff_list.count() ; ++i )
	{
		for ( DWORD j = 0 ; j < riff_list[ i ].chunk_count ; ++j )
		{
			...
			Data data( chunk.data, chunk.size );
