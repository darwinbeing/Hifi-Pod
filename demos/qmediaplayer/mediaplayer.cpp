...
void MediaPlayer::rewind()
{
     ...
     currentPlayPos--;
     if(currentPlayPos >= 0)
    m_MediaObject.seek(currentPlayPos);
}
...
printf("***%s: %d\n", __FUNCTION__, __LINE__);
    ...
    //     m_MediaObject.play();
    // }
    int Pos = currentPlayPos++ % queue.size();
    
    m_MediaObject.setCurrentSource(queue[Pos]);
    forwardButton->setEnabled(queue.size() > 1);
    m_MediaObject.play();
