...
#define AM335X_TS_XMAX                 0xFF0
#define AM335X_TS_YMIN                 0xDC
//#define AM335X_TS_YMAX                 0xF43
#define AM335X_TS_YMAX                 0xFF3

#endif

/* Calibration data, 7 parameters */
static int cal_data[7] = {0, 1, 0, 0, 0, 1, 1};
			...
			if (pen == 0) {
				if ((diffx < 15) && (diffy < 15)
						&& (z <= MAX_12BIT)) {
			                ...
			                cal_x = (cal_data[0] + cal_data[1]*val_x + cal_data[2]*val_y) / cal_data[6];
                			cal_y = (cal_data[3] + cal_data[4]*val_x + cal_data[5]*val_y) / cal_data[6];
...
//                     printk("x: %d y: %d, calx: %d caly: %d\n", val_x, val_y, cal_x, cal_y);
...
int convert_calibrate_str(const char *str, int *data)
{
        int i = 0;
        int j = 0;
        char *substr[7] = {NULL};
        ...
        for(i=0; i<strlen(str); i++)
        {
                ...
                {
                        buf[i] = '\0';
                        if(j>=7) return -1; else substr[j++] = &buf[i+1];
                }
        }
        ...
        for(j=0; j<7; j++)
        {
                if(strict_strtol(substr[j], 10, &temp))
                        return -1;
                else
                        data[j] = (int)temp;
        }
...
static ssize_t tsc_calibrate_show(struct device *dev,
                                     struct device_attribute *attr, char *buf)
{
        return sprintf(buf, "%d %d %d %d %d %d %d\n",
                cal_data[0], cal_data[1], cal_data[2], cal_data[3],
                cal_data[4], cal_data[5], cal_data[6]);
}

static ssize_t tsc_calibrate_store(struct device *dev,
                                     struct device_attribute *attr,
                                     const char *buf, size_t count)
{
        if (convert_calibrate_str(buf, cal_data))
                return -EINVAL;

        return count;
}

static DEVICE_ATTR(calibrate, 0664, tsc_calibrate_show, tsc_calibrate_store);

static struct attribute *tsc_attributes[] = {
        &dev_attr_calibrate.attr,
        NULL,
};

static struct attribute_group tsc_attr_group = {
        .attrs = tsc_attributes,
};
...
static	int __devinit tscadc_probe(struct platform_device *pdev)
{
	...
	input_set_abs_params(input_dev, ABS_PRESSURE, 0, MAX_12BIT, 0, 0);
        ...
        err = sysfs_create_group(&pdev->dev.kobj, &tsc_attr_group);
        if (err)
                goto err_fail;
